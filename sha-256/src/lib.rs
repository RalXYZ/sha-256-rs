...
impl Sha256 {
    pub fn do_hash(&mut self, data: &[u8]) -> [u8; 32] {
        let mut h: [u32; 8] = [
...
mod tests {
    ...
    fn short_sha_256() {
        let mut test = Sha256 {};
        assert_eq!(
            test.do_hash(b"abc").to_vec(),
            hex::decode("ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad")
                .unwrap()
        );
        assert_eq!(
            test.do_hash(b"blockchain").to_vec(),
    ...
    fn long_sha_256() {
        let mut test = Sha256 {};
        assert_eq!(
            test.do_hash(b"The quick brown fox jumps over the lazy dog")
                .to_vec(),
